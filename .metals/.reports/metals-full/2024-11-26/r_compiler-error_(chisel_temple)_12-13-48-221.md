...
### scala.reflect.internal.FatalError: no context found for source-file://<WORKSPACE>/src/main/scala/SRAM/sram.scala.scala,line-1,offset=9
...
//data array：包含多个SRAM
class data_array(cache_line_number:Int,data_block_size:Int,width: Int=8,numReadPorts: Int=2, numWritePorts: Int=2, numReadwritePorts: Int=0)extends Module{

    val io = IO(new Bundle{
      val port=Vec(cache_line_number,new SRAMInterface(data_block_size, UInt(width.W), numReadPorts, numWritePorts, numReadwritePorts))
    })
    
    for(i<-0 until cache_line_number-1){
        io.port(i) :<>= SRAM(data_block_size, UInt(width.W), numReadPorts, numWritePorts, numReadwritePorts)
    }    
